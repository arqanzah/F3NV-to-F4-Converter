...
#ifndef LIB_NIF_FO3_TYPES_H_
#define LIB_NIF_FO3_TYPES_H_
...
namespace nif::fo3 {
	...
	public:
		...
		friend std::ostream& operator<<(std::ostream& stream, const ReferenceInterface& ref) {
			stream << ref.index;
			return stream;
		}
	...
	template<typename T>
	class Reference : public ReferenceInterface {
		T* object = nullptr;
	...
	class NifPropertyInterface {

		const std::string name;

	private:
		NifPropertyInterface() {}
		NifPropertyInterface(const NifPropertyInterface&) = delete;

	protected:
		NifPropertyInterface(std::string name) : name(name) {}

	public:
		virtual ~NifPropertyInterface() {}
	...
	template<typename T>
	class NifProperty : public NifPropertyInterface {
	...
	private:
		NifProperty() {}
		NifProperty(const NifProperty&) = delete;

	public:
		NifProperty(std::string name, T value,  std::function<bool()> condition = []() { return true; }) : NifPropertyInterface(name), value(value), condition(condition) {};
		~NifProperty() {}
		...
		void write(NiStream& stream) override {
			if (condition()) {
				stream << value;
			}
		};
	...
	public:
		...
		NiArray(const unsigned int& sizePtr) : sizePtr(&sizePtr) {
		...
		friend NiStream& operator<<(NiStream& stream, const NiArray& dt) {
			stream << *dt.sizePtr;
			return stream;
		}
	...
	class NifObject : public NiObject {

		std::vector<std::unique_ptr<NifPropertyInterface>> properties;

	protected:
		NifObject() {}

		template<typename T>
		NifProperty<T>* addProperty(const std::string& name, const T& value, std::function<bool()> condition = []() { return true; }) {

			properties.push_back(std::make_unique<NifProperty<T>>(name, value, condition));
			return static_cast<NifProperty<T>*>(properties[properties.size()-1].get());
		}
	...
	public:
		...
		template<typename T>
		NifProperty<T>* getProperty(const std::string& name) {
			for (const std::unique_ptr<NifPropertyInterface>& prop : properties) {
				if (prop->getName() == name) {
					return static_cast<NifProperty<T>*>(prop.get());
				}
			}
			return nullptr;
		}
	...
	class NifObjectNet : public NifObject {
	public:
		NifObjectNet() : NifObject() {
			addProperty("nameRef", int(-1));
		}

	};

	class NifNode : public NifObjectNet {
	public:
		...
		NifNode() : NifObjectNet() {
			addProperty("numAdditionalData", (unsigned int)(0));
			addProperty("array", NiArray<std::string>(getProperty<unsigned int>("numAdditionalData")->getValue()));
		}

	};


		nif::fo3::NifNode node;
	node.getProperty<int>("nameRef")->setValue(2);

	node.getProperty<nif::fo3::NiArray<std::string>>("array")->getValue().get()->push_back("sdsd");
